...
{
    fn decapsulate2(&self, encapsulated_key: &GenericArray<u8, K2::CiphertextSize>) -> Result<Array<u8, L>, 
        <K2::Decapsulator as Decapsulate<GenericArray<u8, K2::CiphertextSize>, Array<u8, K2::SharedKeySize>>>::Error>
    {
        ...
        let ss3 = X963Kdf::<Sha256>::derive_secrets_other([self.intermediate_key.as_slice(), &ss], &[]);
        Ok(ss3)
    }
}
...
{
    fn recapsulate(&self, rng: &mut impl CryptoRngCore, encapsulated_key: &GenericArray<u8, K1::CiphertextSize>)
        ...
        //<K2::Decapsulator as Decapsulate<GenericArray<u8, K2::CiphertextSize>, Array<u8, K2::SharedKeySize>>>::Error>
    {
        ...
        let ss3 = X963Kdf::<Sha256>::derive_secrets_other([ss.as_slice(), &ss2], &[]);
        Ok((ss3, ek2))
    }
}
...
fn test_mkem_1() 
{
    ...
    let (ss_u, ek_v) = v.recapsulate(&mut OsRng, &ek_u).unwrap();
    let ss_v = u_decap.decapsulate2(&ek_v).unwrap();
...
fn test_mkem_2() 
{
    ...
    let (ss_u, ek_v) = v.recapsulate(&mut OsRng, &ek_u).unwrap();
    let ss_v = decap_u.decapsulate2(&ek_v).unwrap();
...
{
    fn decapsulate2(&self, encapsulated_key_static: &GenericArray<u8, K1::CiphertextSize>, encapsulated_key_ephem: &GenericArray<u8, K2::CiphertextSize>) -> Result<Array<u8, L>, 
         <K2::Decapsulator as Decapsulate<GenericArray<u8, K2::CiphertextSize>, Array<u8, K2::SharedKeySize>>>::Error>
    //fn encapsulate2(&self, ephem_public_key: &GenericArray<u8, <<K2 as Capsulator>::Encapsulator as EncodedSizeUser2>::EncodedSize>) -> Result<Array<u8, L>, 
    {
        ...
        let ss3 = X963Kdf::<Sha256>::derive_secrets_other([ss1.as_slice(), &ss2], &[]);
        Ok(ss3)
    }
}
...
where K2::Encapsulator: EncodedSizeUser2
{
    fn recapsulate(&self, rng: &mut impl CryptoRngCore, ephem_public_key: &GenericArray<u8, <<K2 as Capsulator>::Encapsulator as EncodedSizeUser2>::EncodedSize>)
...
fn test_mkem_1s1e_1() 
{
    ...
    // let ss_u = u.decapsulate(&ek_v, false);
    // let ss_v = v.decapsulate(&ek_u, true);
    let (ss_u, ek_v, ek_ephem) = v.recapsulate(&mut OsRng, &pk_ephem).unwrap();
    let ss_v = u_decap.decapsulate2(&ek_v, &ek_ephem).unwrap();
...
fn test_mkem_1s1e_2() 
{
    ...
    let (ss_u, ek_v, ek_ephem) = v.recapsulate(&mut OsRng, &ek_u).unwrap();
    let ss_v = decap_u.decapsulate2(&ek_v, &ek_ephem).unwrap();
